  ...
  class Clients_trading_model extends CI_Model {
    ...
    public function update_balances($user, $bank, $amount, $currency) {
      ...
      $this->db->set("amount", "amount + {$amount}", false)->where(array("banks_id"=>$user, "currencies_id"=>$currency))->update("banks_balances");
		...
		function get_user_corporate_amount ($user_id) {
			
			$amount = array ();
			for ($pair_id = 1; $pair_id <= 2; $pair_id++) {
				$this->db->from ("users_corporate_amount");
				$this->db->where (array('user_id' => $user_id, 'pair_id' => $pair_id));
				$query = $this->db->get ();
				if ($query->num_rows == 0) {
					$this->db->insert ("users_corporate_amount", array('user_id' => $user_id, 'pair_id' => $pair_id,
																			'sell' => 0, 'buy' => 0));
					$amount[$pair_id]['sell'] = 0;					
					$amount[$pair_id]['buy'] = 0;
				}
		...
		function get_total_day_corporate_amount ($user_id, $pair_id, $deal) {
			$this->db->from ("users_corporate_amount");
			$this->db->where (array ('user_id' => $user_id, 'pair_id'=> $pair_id));			
			$query = $this->db->get ();
			if ($query->num_rows () == 0) {
		...
		function get_user_total_amount ( $user_id ) { 
			...
			$retail = $this->get_user_amount($user_id); 
			$corporate = $this->get_user_corporate_amount($user_id); 
		
			for( $pair_id = 1 ; $pair_id <= 2 ; $pair_id++ ) {
			
				$fx_total[$pair_id] = $retail[$pair_id]['sell'] + $corporate[$pair_id]['sell'] + $retail[$pair_id]['buy'] + $corporate[$pair_id]['buy'] ;
			}
